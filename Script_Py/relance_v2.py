...
def read_excel_file_endowment(file_path):
    global filtered_df  # Ajoutez cette ligne pour s'assurer que filtered_df est global
    ...
    for index, row in filtered_df.iterrows():
        ...
        # Utilisation de place pour positionner le bouton correctement
        copy_button = ttk.Button(email_listbox_endowment, text="Copier",
                                 command=lambda text=email_text: copy_to_clipboard(text))
        copy_button.place(x=500, y=index * 20)
    return filtered_df
...
def send_emails():
    ...
    for i, entry in enumerate(selected_emails):
        ...
        try:
            ...
            df.loc[(df['Matériel concerné'] == materiel) & (
                    df['Mail'] == mail_to), 'Etape'] = f"Etape {int(etape.split()[1]) + 1}"
...
email_listbox_endowment = tk.Listbox(frame_endowment, selectmode=tk.MULTIPLE, height=20, width=100)
